...
public class Press extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<Press> CODEC = createCodec(Press::new);
    public static final BooleanProperty ACTIVE = BooleanProperty.of("active");

    @Override
    public MapCodec<Press> getCodec() {
        return CODEC;
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        if (type == ModBlockEntities.PRESS) {
            return (world1, pos, state1, blockEntity) -> {
                if (blockEntity instanceof PressBlockEntity press) {
                    press.tick(world1, pos, state1);
                }
            };
        }
        return null;
