...
public class CleanerScreenHandler extends ScreenHandler {
    ...
    public CleanerScreenHandler(int syncId, PlayerInventory inventory, BlockPos pos) {
        this(syncId, inventory, inventory.player.getEntityWorld().getBlockEntity(pos), new ArrayPropertyDelegate(2));
    }

    public CleanerScreenHandler(int syncId, PlayerInventory playerInventory,
                                BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(ModScreenHandlers.CLEANER_SCREEN_HANDLER, syncId);
        this.inventory = ((Inventory) blockEntity);
        this.blockEntity = ((CleanerBlockEntity) blockEntity);
    ...
    public long getRealRemainingTicks() {
        if (blockEntity instanceof CleanerBlockEntity) {
            CleanerBlockEntity splavEntity = (CleanerBlockEntity) blockEntity;
