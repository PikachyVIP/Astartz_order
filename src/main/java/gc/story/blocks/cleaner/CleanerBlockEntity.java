...
public class CleanerBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos>, ImplementedInventory {
    ...
    public CleanerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CLEANER, pos, state);
    ...
    public Text getDisplayName() {
        return Text.translatable("block.story.cleaner");
    }

    @Nullable
    @Override
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new CleanerScreenHandler(syncId, playerInventory, this, this.propertyDelegate);
    }
    ...
    protected void writeData(WriteView view) {
        ...
        view.putInt("cleaner.progress", progress);
        view.putInt("cleaner.max_progress", maxProgress);
        view.putInt("cleaner.real_progress", realProgress);
        view.putInt("cleaner.multiplier", progressMultiplier);
    }

    @Override
    protected void readData(ReadView view) {
        Inventories.readData(view, inventory);
        progress = view.getInt("cleaner.progress", 0);
        maxProgress = view.getInt("cleaner.max_progress", 0);
        realProgress = view.getInt("cleaner.real_progress", 0);
        progressMultiplier = view.getInt("cleaner.multiplier", 1);
        super.readData(view);
    }
    ...
    public void tick(World world, BlockPos pos, BlockState state) {
        if (hasRecipe()) {
            ...
            world.setBlockState(pos, state.with(Cleaner.ACTIVE, true));

            if (hasCraftingFinished()) {
                craftItem();
                resetProgress();
                currentRecipe = null;
                world.setBlockState(pos, state.with(Cleaner.ACTIVE, false));
            }
        } else {
            resetProgress();
            world.setBlockState(pos, state.with(Cleaner.ACTIVE, false));
    ...
    private boolean hasRecipe() {
        ...
        BlockRecipes.Recipe newRecipe = BlockRecipes.getCleanerRecipe(inputStack.getItem());
